...
import { AGENTS, MODEL_CATALOG, PROVIDER_LABELS, modelInfo } from "@/lib/rq/agents";
...
function Editor({ agentId, requirementId, runningSnapshotVersion, compact, scope }: Props & { scope: Scope }) {
  const { profiles, requirements, saveProfile, clearOverride } = useRq();
  ...
  const save = () => {
    saveProfile(agentId, { agentId, ...draft }, note, scope === "requerimiento" ? requirementId : undefined);
    setNote("");
    setSaved(true);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input value={note} onChange={(e) => setNote(e.target.value)} placeholder={promptDirty ? "Nota de la versión (qué cambiaste y por qué)" : "Nota (opcional)"} className={`${input} min-w-[200px] flex-1`} />
        ...
        <button className="btn-primary py-1.5" disabled={!dirty} onClick={save}>
          {saved ? "Guardado ✓" : promptDirty ? `Guardar como v${Math.max(...source.versions.map((v) => v.version)) + 1}` : "Guardar"}
        </button>
      </div>
      {testing === "done" && (
